...
This drive was approximately $13 as of March 2019.
...
### Python 3 & Libraries
Additionally, the floppy disk must contain a Python 3 file, entry.py, which implements a standardized interface through flask (detailed below and in the _samples_ folder).
...
See the *docker_image* folder in SkyPond for more information.
...
## Submission Overview
This is a detailed summary of what the executing environment looks like, what is expected from your submission, and a summary of how the platform works from a technical perspective.

See the *examples* folder for sample submissions that are acceptable with minimal modification.

### Entry File
The only required file is **handler.py**. This must contain a flask app exposing two routes:

* /information
* /react

There are working versions of the routes in the *examples* folder, and it's strongly recommended that you copy an existing sample to start.

As long as you implement these methods to spec in some capacity, we should be able to (attempt to) validate your submission.
...
* Your bot, during the competition, is given approximately 500ms to return a result through the *react* method.
